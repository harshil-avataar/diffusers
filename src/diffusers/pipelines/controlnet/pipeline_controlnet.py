...
from model.mesh_fusion.render import render_mesh
...
class StableDiffusionControlNetPipeline(
    ...
    def prepare_latent_for_texture(self,texture,dtype, device, generator ):
        # STATE: Omitted std normalization

        latents = randn_tensor((texture.shape[0],texture.shape[1],4),generator=generator, device=device, dtype=dtype)

        return latents

    def render_from_mesh(self,mesh,latents,poses,size):


        for curr_pos in range(len(poses)):
            img = render_mesh(mesh,self.args,size,size,poses[curr_pos],None,1)[0]

            latents[curr_pos,:] = img[:4]

            # print(latents.mean(),latents.std(),latents.min(),latents.max())

            # add std normalization
            # latents[curr_pos,...] =  latents[curr_pos,...]  * self.scheduler.init_noise_sigma / (latents [curr_pos].std())
        
        return latents.to('cuda')


    def calc_backprojected_texture(self, noised_mesh, latents, poses,i):

        from pytorch3d.renderer import (
            FoVPerspectiveCameras, PerspectiveCameras)
        from model.mesh_fusion.util import get_camera
        from PIL import Image
        from torch.nn.functional import interpolate


        latents_interpolated = interpolate(latents, scale_factor= 1)
        print(latents_interpolated.shape)
        faces_uvs, verts_uvs, init_texture = utils.compute_texture_and_uvs(noised_mesh)

        generate_mask_image = Image.fromarray((np.ones((512,512)) * 255.).astype(np.uint8))
        init_texture = torch.zeros(init_texture.shape).to(init_texture)
        count_texture = torch.zeros(init_texture.shape).to(init_texture)

        
        for curr_pos in range(latents.shape[0]):

            c2w = poses[curr_pos].inverse()
            cameras = get_camera(c2w,55)

            init_texture,count_texture = backproject_latent_mesh(noised_mesh, faces_uvs, verts_uvs, cameras, 
                latents_interpolated[curr_pos], generate_mask_image, generate_mask_image, 
                init_texture,  count_texture, 
                'cuda', 1024)
        
        init_texture = torch.where(count_texture > 0, init_texture / count_texture, init_texture)
        # Image.fromarray((init_texture.detach().cpu().numpy()*255).astype(np.uint8)).save(f'output/diff/texture_{i}.png')
        return TexturesUV(
                maps = init_texture[None, ...],
                faces_uvs = faces_uvs[None, ...],
                verts_uvs = verts_uvs[None, ...]
            )
    
    # def scale_0_1(self,latent):

    def save_mesh(self,mesh,name = 'temp' ):
        from pytorch3d.io import save_obj
        
        # texture = TexturesUV(mesh.textures._maps_padded[...,:3],
        #         faces_uvs = mesh.textures._faces_uvs_padded,
        #         verts_uvs = mesh.textures._verts_uvs_padded)
        
                # mesh._verts_padded[0],mesh._faces_padded[0])

        save_obj(
            f"{name}.obj",
            verts=mesh._verts_padded[0],
            faces=mesh._faces_padded[0],
            decimal_places=5,
    ...
    def __call__(
        ...
        # CURRENT STATE:
        # Standard deviation is changing when projecting, think what to do.
        # observation: with texture resolution, the unprojection and projection is also affected.


        # TODO: 
        # 1. Try the projection etc code written here for normal mesh and visualise latents etc ... 
        # 
        #  
        self.args = args
        latent_dim = 512 // self.vae_scale_factor
        ...
        faces_uvs, verts_uvs, texture = utils.compute_texture_and_uvs(mesh) 
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # latents2 = self.render_from_mesh(mesh_noised,latents.clone(),poses,64) # (poses,4,64,64)
        ...
        if not output_type == "latent":
            ...
            # latents = self.render_from_mesh(mesh_noised,latents,poses,64)
            images = self.vae.decode(latents / self.vae.config.scaling_factor, return_dict=False)
            image_pil = self.image_processor.postprocess(images[0], output_type='pil')
        
            
            for i in range(4):
                image_pil[i].save(f'imgn_{i}.png')

            image, has_nsfw_concept = self.run_safety_checker(images[0], device, prompt_embeds.dtype)
        else:
            image = latents
            has_nsfw_concept = None
