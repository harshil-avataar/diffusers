...
class StableDiffusionPanoramaPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    ...
    def __call__(
        ...
        # views = views[:2]
        views = [(0, 64, 0, 64), (0, 64, 32, 96)]
        # views = [(0, 64, 0, 64), (0, 64, 56, 120)]
        print(views)
        # exit()
        view_batch_size = 2
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for j, batch_view in enumerate(views_batch):
                    ...
                    # latents_temp[j, ...] = latents_view_denoised[0,...]


                    latents[...,:32] = latents_denoised_batch [0,...,:32]
                    latents[...,32:64] =  ( latents_denoised_batch[0,..., 32:] + latents_denoised_batch[1, ..., :32])/ 2
                    # latents[0,:,:,30:] += (mask_tensor_2*latents[1,...])[0,...,:34] / 2
                    latents[0,...,64:] = latents_denoised_batch[1,...,32:]
                    

                # take the MultiDiffusion step. Eq. 5 in MultiDiffusion paper: https://arxiv.org/abs/2302.08113

                # latents[...,:32] = latents_temp [0,...,:32]
                # latents[...,32:64] =  ( latents_temp[0,..., 32:] + latents_temp[1, ..., :32])/ 2
                # # latents[0,:,:,30:] += (mask_tensor_2*latents[1,...])[0,...,:34] / 2
                # latents[0,...,64:] = latents_temp[1,...,32:]
